...
const Settings = () => {
  ...
  // The (app) route-group layout already guarantees an authenticated,
  // onboarded session server-side. This only covers the brief client-side
  // window before AuthContext has loaded — redirecting here would be a
  ...
  const toggleChallenge = async (id: Challenge) => {
    ...
    const newChallenges = current.includes(id)
    ...
    const { error } = await updateProfile({ selected_challenges: newChallenges });
    if (error) {
